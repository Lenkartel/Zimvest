// api/register.js — ZimVest
// Creates a new user: phone + bcrypt-hashed password → Vercel KV
// Issues a 30-day signed JWT session cookie on success.
...
export default async function handler(req, res) {
  ...
  if (!/^\+263[0-9]{9}$/.test(phone))
    return res.status(400).json({ error: 'Invalid EcoCash number. Format: +2637XXXXXXXX' });
